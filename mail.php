<?php
$VotreAdresseMail="chromatykretro@gmail.com";//mettez ici votre adresse mail
        ...
        if(!preg_match("#^[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?@[a-z0-9_-]+((\.[a-z0-9_-]+){1,})?\.[a-z]{2,}$#i",$_POST['email'])){
            echo "L'adresse mail entrée est incorrecte";
                ...
                if(empty($_POST['message'])) {
                    ...
                    $Mail=$_POST['email'];
                    $Sujet='=?UTF-8?B?'.base64_encode('Sujet Test').'?=';//Cet encodage (base64_encode) est fait pour permettre aux informations binaires d'être manipulées par les systèmes qui ne gèrent pas correctement les 8 bits (=?UTF-8?B? est une norme afin de transmettre correctement les caractères de la chaine)
